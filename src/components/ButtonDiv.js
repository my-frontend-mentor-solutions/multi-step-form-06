...
export default function ButtonDiv(props) {
  ...
  return (
    <div className='lg:m-0 lg:mt-14 lg:p-0 lg:static lg:mx-10 lg:w-auto flex items-center fixed left-0 bottom-0 bg-white w-screen justify-between p-5'>
      ...
      {
        props.nextRoute && <Link type="button" className='text-extra-light-gray bg-marine-blue px-[18px] py-[10px] rounded-lg hover:bg-marine-light-blue duration-200' to={"/"+props.nextRoute} >{ props.final ? "Confirm" : "Next Step" }</Link>
      }
    </div>
  )
}
